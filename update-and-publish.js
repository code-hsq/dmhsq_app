...
function updatePackage(packageName, type) {
  ...
  if (packageJson.dependencies) {
    Object.keys(packageJson.dependencies).forEach((dep) => {
      if (dep.startsWith('@dmhsq_app/')) {
        const depName = dep.replace('@dmhsq_app/', '');
  ...
  const defaultChangelogEntries = {
    // utils: ['优化工具函数', '提高兼容性和稳定性', '改进正则表达式模式'],
    // core: ['优化事件处理机制', '提升日志记录效率', '更新依赖包版本'],
    // processor: [
    //   '改进IP数据收集机制',
    //   '优化错误数据去重算法',
    //   '增强数据处理性能',
...
function updateConstantsVersion(packageName, newVersion) {
  ...
  const newContent = content.replace(
    pattern,
    `export const ${config.varName} = '${newVersion}'`
  );

  if (content === newContent) {
    log(
      `警告：在 ${constantsPath} 中没有找到 ${config.varName} 常量`,
      colors.yellow
    );
  } else {
    fs.writeFileSync(constantsPath, newContent, 'utf8');
    log(
      `更新了 ${constantsPath} 中的 ${config.varName} -> ${newVersion}`,
      colors.green
    );
  }
}
...
function buildPackage(packageName) {
  ...
  try {
    execSync(`cd packages/${packageName} && npm run build`, {
      stdio: 'inherit',
    });
